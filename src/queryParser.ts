...
export function queryParser(req: Express.Request, res: Response, next: NextFunction) {
    try {
...
function validateFilter(filter: Record<string, any>): void {
...
function parseRange(range: any): QueryRange {
    if (!range) return {};
...
function validateRange(range: any): void {
...
function parseSort(sort: any): QuerySort[] | undefined {
    ...
    try {
        ...
        return sortArray.map((s): QuerySort => {
...
function parsePage(page: any): number {
    const defaultPage = 1;
...
function parseLimit(limit: any): number {
...
function parseRelations(relations: any): string[] {
    if (!relations) return [];
