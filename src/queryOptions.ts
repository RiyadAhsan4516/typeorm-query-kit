...
interface QueryOptions<T extends ObjectLiteral> {
    ...
    range?: Record<string, { gt?: number; gte?: number; lt?: number; lte?: number }>;
 ...
 */
export async function applyQueryOptions<T extends ObjectLiteral>(
...
function isRelationJoined(
    qb: SelectQueryBuilder<any>,
    parentAlias: string,
    relationAlias: string
): boolean {
    return qb.expressionMap.joinAttributes.some(
        j => j.alias.name === relationAlias && j.parentAlias === parentAlias
    );
}
...
function applyRanges(
    ...
    for (const [field, conditions] of Object.entries(ranges)) {
        ...
        if (conditions.gt !== undefined) qb.andWhere(`${alias}.${fieldName} > :${paramBase}_gt`, { [`${paramBase}_gt`]: conditions.gt });
        if (conditions.gte !== undefined) qb.andWhere(`${alias}.${fieldName} >= :${paramBase}_gte`, { [`${paramBase}_gte`]: conditions.gte });
        if (conditions.lt !== undefined) qb.andWhere(`${alias}.${fieldName} < :${paramBase}_lt`, { [`${paramBase}_lt`]: conditions.lt });
        if (conditions.lte !== undefined) qb.andWhere(`${alias}.${fieldName} <= :${paramBase}_lte`, { [`${paramBase}_lte`]: conditions.lte });
    ...
    for (const [op, value] of Object.entries(operators)) {
        switch (op) {
            ...
            case '$LIKE':
                ...
                break;
            case '$in':
                if (!Array.isArray(value)) throw new Error(`$in value must be an array`);
                qb.andWhere(`${alias}.${fieldName} IN (:...${paramBase}_in)`, { [`${paramBase}_in`]: value });
                break;
            case '$nin':
                if (!Array.isArray(value)) throw new Error(`$nin value must be an array`);
...
function resolveAlias(field: string, baseAlias: string): { alias: string; fieldName: string } {
...
function generateParamKey(alias: string, fieldName: string): string {
    paramCounter++;
...
function isFieldAllowed(alias: string, fieldName: string, operation: keyof FieldConfig, config: EntityQueryConfig): boolean {
...
function isFieldSearchable(alias: string, fieldName: string, config?: EntityQueryConfig): boolean {
